...
- [2026-08-22]: Kiến trúc tối ưu vòng đời tải trang (Page Load Lifecycle) & Bảo mật phân quyền: Sử dụng Server-Side Data Injection (SSR) nhúng sẵn `initialData` vào template `doGet`, kết hợp CacheService 2 lớp (Shared Metadata Cache 60m & User-isolated Permission Cache 15m) và Centralized Auth Gate (`API_SECRET_TOKEN`) ở đầu mọi route Core API. Lý do: Triệt tiêu hiện tượng đa chặng mạng (Multi-hop Waterfall Latency: Client -> Proxy -> Core API -> Sheets) giúp mở trang tức thì < 500ms mà không làm suy yếu ranh giới bảo mật Service-to-Service.
