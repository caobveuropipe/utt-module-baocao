...
let _sheetDataChotCong;
function getSheetDataChotCong() {
  if (!_sheetDataChotCong) _sheetDataChotCong = SpreadsheetApp.openById(idFileChotCong).getSheetByName('DataChotCong');
  return _sheetDataChotCong;
}

const idFileDataAnCa = '1-pKqPF-GpmoTXno1no5NN-JRIMLsHwBqGx1xdLHPgr4';//LibraryDigiCore.idFileDataAnCa;
const sheetDataAnCa = SpreadsheetApp.openById(idFileDataAnCa).getSheetByName('Database');
  ...
  try {
    ...
    if (action === 'delete') {
      ...
      if (strReturn === 'Success') {
        LibraryDigiCore.logDiary('DataAnCa', '', 1, 'xóa-all-data', userEmail, '');
      ...
      if (strReturn === 'Success') {
        try {
          LibraryDigiCore.logDiary('DataAnCa', '', 1, 'print-data', userEmail, '');
        } catch (err) {
          return ContentService.createTextOutput(JSON.stringify({
            status: "error",
      ...
      try {
        LibraryDigiCore.logDiary('DataThuyetMinhL2', '', undefined, 'thêm', userEmail, '...');
      } catch (e) {
...
function delAllData(month) {
  ...
  const lastChotCongRow = getSheetDataChotCong().getLastRow();

  if (lastChotCongRow >= 2) {
    const ccData = getSheetDataChotCong().getRange(2, 1, lastChotCongRow - 1, 13).getValues(); // A → M
    let ccFlag = false;
    let ccExtraCount = 0;
...
function updateStatusTinhLuong(month, newStatus) {
  ...
  for (let i = dmData.length - 1; i >= 0; i--) {
    ...
    if (rowMonth == month) {
      // Cập nhật giá trị mới vào cột H (index 7)
      sheetDanhMucThang.getRange(i + 1, 14).setValue(newStatus);
      return 'success';
    }
  }
